...
import { COMMUNITIES, initials } from "@/lib/community-data";
import { useJoined } from "@/lib/community-store";
...
function CreatePost() {
  ...
  const options = COMMUNITIES.filter((c) => joinedIds.includes(c.id));
  const fallback = options[0]?.id ?? COMMUNITIES[0]!.id;
    ...
    <MobileShell tone="white">
      <header className="sticky top-0 z-20 flex items-center gap-3 border-b border-border bg-background/95 px-4 py-3 backdrop-blur">
        ...
        <button
          ...
          disabled={!canPublish}
          onClick={publish}
          className="rounded-full bg-primary px-4 py-2 text-[13px] font-bold text-primary-foreground transition-all duration-200 active:scale-95 disabled:opacity-40"
        >
          Publish
        </button>
      </header>
        ...
        <div className="flex items-center gap-3">
          ...
          <label className="min-w-0 flex-1">
            ...
            <select
              ...
              className="h-9 w-full rounded-xl border border-input bg-background px-3 text-[13.5px] font-semibold text-foreground outline-none transition-colors focus:border-primary"
            >
              {(options.length ? options : COMMUNITIES).map((c) => (
        ...
        {poll ? (
          <div className="animate-fade-up mt-4 rounded-2xl border border-border bg-surface p-3">
            <div className="mb-2 flex items-center justify-between">
              <span className="text-[12.5px] font-extrabold text-foreground">Poll (preview)</span>
              <button
                type="button"
            ...
            <div className="space-y-2">
              {poll.map((opt, i) => (
                <input
                  ...
                  className="h-10 w-full rounded-xl border border-input bg-card px-3 text-[13.5px] outline-none focus:border-primary"
                />
              ))}
            </div>
            <button
      ...
      <div className="sticky bottom-0 border-t border-border bg-background/95 px-5 py-3 pb-[max(env(safe-area-inset-bottom),0.75rem)] backdrop-blur">
        <button
          ...
          disabled={!canPublish}
          onClick={publish}
          className="h-13 w-full rounded-2xl bg-navy-gradient text-[15px] font-bold text-primary-foreground shadow-lift transition-all duration-200 active:scale-[0.98] disabled:opacity-40 disabled:shadow-none"
        >
          Publish post
        </button>
      </div>
...
function AttachButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        "flex flex-col items-center gap-1.5 rounded-2xl border border-border bg-card py-3 text-[12px] font-bold text-foreground transition-all duration-200 hover:shadow-soft active:scale-95",
      )}
    >
